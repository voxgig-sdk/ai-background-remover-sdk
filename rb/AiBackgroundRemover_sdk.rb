...
require_relative 'AiBackgroundRemover_types'
...
class AiBackgroundRemoverSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = AiBackgroundRemoverHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, AiBackgroundRemoverFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    AiBackgroundRemoverUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => AiBackgroundRemoverError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue AiBackgroundRemoverError => err
    ...
    if fetched.is_a?(Hash)
      status = AiBackgroundRemoverHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = AiBackgroundRemoverError.new(
        "graphql_error", "AiBackgroundRemoverSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = AiBackgroundRemoverSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
