...
class BackgroundRemovalEntity
  ...
  def make
    opts = @_entopts.dup
    BackgroundRemovalEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.clone(args)) || {}
    ...
    _run_op(ctx) do
      if ctx.result
        if ctx.result.resdata
          @_data = AiBackgroundRemoverHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
