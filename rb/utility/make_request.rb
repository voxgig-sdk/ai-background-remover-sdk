...
module AiBackgroundRemoverUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = AiBackgroundRemoverResponse.new({})
    ...
    if fetch_err
      ...
      response = AiBackgroundRemoverResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = AiBackgroundRemoverResponse.new(fetched)
    else
