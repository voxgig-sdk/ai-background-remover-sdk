// Command AiBackgroundRemover-cli is a CLI + REPL for the AiBackgroundRemover SDK,
...
const prompt = "ai-background-remover"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "background_removal"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      AI_BACKGROUND_REMOVER_APIKEY (api key), AI_BACKGROUND_REMOVER_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: AI_BACKGROUND_REMOVER_APIKEY carries the API key and
	// AI_BACKGROUND_REMOVER_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("AI_BACKGROUND_REMOVER_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("AI_BACKGROUND_REMOVER_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewAiBackgroundRemoverSDK(opts)
