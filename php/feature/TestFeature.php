...
// AiBackgroundRemover SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class AiBackgroundRemoverTestFeature extends AiBackgroundRemoverBaseFeature
{
    ...
    public function init(AiBackgroundRemoverContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (AiBackgroundRemoverContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = AiBackgroundRemoverParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (AiBackgroundRemoverContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(AiBackgroundRemoverContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = AiBackgroundRemoverParam::call($ctx, $k);
