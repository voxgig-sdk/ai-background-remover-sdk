...
// AiBackgroundRemover SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = AiBackgroundRemoverOmni::makeStructRunner(
    ...
    {
        return AiBackgroundRemoverOmni::objmodel(AiBackgroundRemoverOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return AiBackgroundRemoverOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return AiBackgroundRemoverOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(AiBackgroundRemoverOmni::structfix($now, Struct::undef()))
                        !== json_encode(AiBackgroundRemoverOmni::structfix($val, Struct::undef()))) {
