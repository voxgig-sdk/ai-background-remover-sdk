...
class BackgroundRemovalEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set AI_BACKGROUND_REMOVER_TEST_BACKGROUND_REMOVAL_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $background_removal_ref01_data_result = $background_removal_ref01_ent->create($background_removal_ref01_data, null);
        $background_removal_ref01_data = Helpers::to_map(is_object($background_removal_ref01_data_result) && method_exists($background_removal_ref01_data_result, 'data_get') ? $background_removal_ref01_data_result->data_get() : $background_removal_ref01_data_result);
...
function background_removal_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("AI_BACKGROUND_REMOVER_TEST_BACKGROUND_REMOVAL_ENTID");
