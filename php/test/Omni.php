...
//    only - a typed AiBackgroundRemoverContext there would make every ctx
...
final class AiBackgroundRemoverOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new AiBackgroundRemoverContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new AiBackgroundRemoverSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new AiBackgroundRemoverResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new AiBackgroundRemoverError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new AiBackgroundRemoverResponse($clean['response']);
    ...
    public static function err_from_map($m): ?AiBackgroundRemoverError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new AiBackgroundRemoverError(is_string($code) ? $code : '', $msg);
    }
