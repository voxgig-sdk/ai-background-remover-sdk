...
//     (\`new AiBackgroundRemoverSDK(...)\` / \`AiBackgroundRemoverSDK.test(...)\` become
//     \`AiBackgroundRemoverSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'AiBackgroundRemoverSDK'
...
const TEST_SEED = {"entity":{"background_removal":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new AiBackgroundRemoverSDK(...)\` and any
// \`AiBackgroundRemoverSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+AiBackgroundRemoverSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/AiBackgroundRemoverSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
