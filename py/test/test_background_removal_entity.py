    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set AI_BACKGROUND_REMOVER_TEST_BACKGROUND_REMOVAL_ENTID JSON to run live")
        client = setup["client"]
        ...
        background_removal_ref01_data = helpers.to_map(runner.entity_data(background_removal_ref01_ent.create(background_removal_ref01_data, None)))
...
def _background_removal_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "AI_BACKGROUND_REMOVER_TEST_BACKGROUND_REMOVAL_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("AI_BACKGROUND_REMOVER_TEST_BACKGROUND_REMOVAL_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("AI_BACKGROUND_REMOVER_TEST_LIVE") == "TRUE":
    ...
    _live = env.get("AI_BACKGROUND_REMOVER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("AI_BACKGROUND_REMOVER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
