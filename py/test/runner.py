# AiBackgroundRemover SDK test runner

from __future__ import annotations
import os
import json

from aibackgroundremover_sdk.utility.voxgig_struct import voxgig_struct as vs


class AiBackgroundRemoverTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            AiBackgroundRemoverTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = AiBackgroundRemoverTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = AiBackgroundRemoverTestRunner.getenv("AI_BACKGROUND_REMOVER_TEST_LIVE")
        override = AiBackgroundRemoverTestRunner.getenv("AI_BACKGROUND_REMOVER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = AiBackgroundRemoverTestRunner.getenv(key)
        ...
        explain = AiBackgroundRemoverTestRunner.getenv("AI_BACKGROUND_REMOVER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["AI_BACKGROUND_REMOVER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if AiBackgroundRemoverTestRunner._test_control is not None:
            return AiBackgroundRemoverTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                AiBackgroundRemoverTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            AiBackgroundRemoverTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = AiBackgroundRemoverTestRunner.load_test_control()
...
def load_env_local():
    AiBackgroundRemoverTestRunner.load_env_local()


def env_override(m):
    return AiBackgroundRemoverTestRunner.env_override(m)


def entity_data(v):
    return AiBackgroundRemoverTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return AiBackgroundRemoverTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return AiBackgroundRemoverTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return AiBackgroundRemoverTestRunner.load_test_control()


def live_client_options():
    return AiBackgroundRemoverTestRunner.live_client_options()


def live_delay_ms():
    return AiBackgroundRemoverTestRunner.live_delay_ms()
