...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("AI_BACKGROUND_REMOVER_TEST_LIVE") == "TRUE" ||
		os.Getenv("AI_BACKGROUND_REMOVER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("AI_BACKGROUND_REMOVER_TEST_EXPLAIN"); explain != "" {
...
type entityTestSetup struct {
	client         *sdk.AiBackgroundRemoverSDK
	data           map[string]any
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.AiBackgroundRemoverSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.AiBackgroundRemoverError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.AiBackgroundRemoverSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.AiBackgroundRemoverError{Msg: msg, Code: code}
}
