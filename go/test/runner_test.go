...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("AIBACKGROUNDREMOVER_TEST_LIVE") == "TRUE" ||
		os.Getenv("AIBACKGROUNDREMOVER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("AIBACKGROUNDREMOVER_TEST_EXPLAIN"); explain != "" {
