...
import (
	...
	omni "github.com/voxgig-sdk/ai-background-remover-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.AiBackgroundRemoverError); is {
				out := map[string]any{
					"name":    "AiBackgroundRemoverError",
...
func omniCtx(arg any, client *sdk.AiBackgroundRemoverSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.AiBackgroundRemoverSDK); is {
			client = live
