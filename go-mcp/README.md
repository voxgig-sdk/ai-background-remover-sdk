...
[MCP](https://modelcontextprotocol.io) server exposing the AiBackgroundRemover SDK as
two agent tools — `ai-background-remover_list` and `ai-background-remover_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/ai-background-remover-mcp)
make build

# 2. Provide credentials via the environment
export AI_BACKGROUND_REMOVER_APIKEY=sk_live_xxx
...
```jsonc
// ai-background-remover_list: first page of records
{ "entity": "background_removal" }
{ "entity": "background_removal", "query": { } }

// ai-background-remover_load: one record by id
{ "entity": "background_removal", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/ai-background-remover-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user ai-background-remover \
     -- "$PWD"/dist/*/ai-background-remover-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `ai-background-remover_list` and `ai-background-remover_load` tools now appear
   in new sessions. Ask the agent to *"list background_removal using ai-background-remover"*
   and it calls `ai-background-remover_list` with `{"entity":"background_removal"}`.
...
```sh
export AI_BACKGROUND_REMOVER_APIKEY=sk_live_xxx            # API key
export AI_BACKGROUND_REMOVER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `ai-background-remover_list` tool
...
### Call the `ai-background-remover_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `ai-background-remover_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `ai-background-remover_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `AI_BACKGROUND_REMOVER_APIKEY` | API key sent with every request. |
| `AI_BACKGROUND_REMOVER_BASE` | Optional override of the API base URL. |
