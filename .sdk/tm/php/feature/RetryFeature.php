...
// AiBackgroundRemover SDK retry feature
...
class AiBackgroundRemoverRetryFeature extends AiBackgroundRemoverBaseFeature
{
    ...
    public function init(AiBackgroundRemoverContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (AiBackgroundRemoverContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(AiBackgroundRemoverContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(AiBackgroundRemoverContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
