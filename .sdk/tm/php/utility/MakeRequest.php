...
class AiBackgroundRemoverMakeRequest
{
    public static function call(AiBackgroundRemoverContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new AiBackgroundRemoverResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new AiBackgroundRemoverResponse($fetched);
        } else {
