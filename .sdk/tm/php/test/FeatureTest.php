...
// AiBackgroundRemover SDK feature test
//
...
class FtCtrl extends AiBackgroundRemoverControl
{
...
class FtClient
{
    ...
    private AiBackgroundRemoverUtility $utility;

    public function __construct(AiBackgroundRemoverUtility $utility, array $options)
    {
    ...
    public function get_utility(): AiBackgroundRemoverUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (AiBackgroundRemoverContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public AiBackgroundRemoverContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (AiBackgroundRemoverContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = AiBackgroundRemoverConfig::make_feature($name);
    ...
    private static function buildUrl(AiBackgroundRemoverSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof AiBackgroundRemoverError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new AiBackgroundRemoverResponse($fetched);
        }
    ...
    private function fail(AiBackgroundRemoverContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (AiBackgroundRemoverContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
