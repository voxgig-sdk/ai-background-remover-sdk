...
// AiBackgroundRemover SDK pipeline test
//
...
{
    private static function utility(): AiBackgroundRemoverUtility
    {
        return new AiBackgroundRemoverUtility();
    }
    ...
    private static function ctx(array $over = []): AiBackgroundRemoverContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new AiBackgroundRemoverContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new AiBackgroundRemoverOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof AiBackgroundRemoverError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new AiBackgroundRemoverOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = AiBackgroundRemoverMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new AiBackgroundRemoverOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = AiBackgroundRemoverMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new AiBackgroundRemoverOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = AiBackgroundRemoverMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AiBackgroundRemoverSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = AiBackgroundRemoverMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new AiBackgroundRemoverResponse([]),
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        [, $err] = AiBackgroundRemoverMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new AiBackgroundRemoverSpec([]),
            'response' => null,
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        [, $err] = AiBackgroundRemoverMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new AiBackgroundRemoverSpec(['step' => 's']),
            'response' => new AiBackgroundRemoverResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        [, $err] = AiBackgroundRemoverMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new AiBackgroundRemoverSpec(['step' => 's']),
            'response' => new AiBackgroundRemoverResponse(self::resp(200, ['v' => 1])),
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        [, $err] = AiBackgroundRemoverMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new AiBackgroundRemoverSpec(['step' => 's']),
            'response' => new AiBackgroundRemoverResponse(self::resp(200, ['v' => 2])),
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        AiBackgroundRemoverMakeResponse::call($ctx);
    ...
    {
        $preset = new AiBackgroundRemoverResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new AiBackgroundRemoverSpec([]),
            'response' => new AiBackgroundRemoverResponse([]),
            'result' => new AiBackgroundRemoverResult([]),
        ...
        [$got, $err] = AiBackgroundRemoverMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new AiBackgroundRemoverResult([])]);
        [, $err] = AiBackgroundRemoverMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new AiBackgroundRemoverSpec([]), 'result' => null]);
        [, $err] = AiBackgroundRemoverMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AiBackgroundRemoverSpec(['step' => 's']),
            'result' => new AiBackgroundRemoverResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new AiBackgroundRemoverOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AiBackgroundRemoverMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new AiBackgroundRemoverSpec(['step' => 's']),
            'result' => new AiBackgroundRemoverResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new AiBackgroundRemoverOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = AiBackgroundRemoverMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new AiBackgroundRemoverResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new AiBackgroundRemoverSpec([]),
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = AiBackgroundRemoverMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AiBackgroundRemoverMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new AiBackgroundRemoverError('boom', 'boom');
        $utility->fetcher = function (AiBackgroundRemoverContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AiBackgroundRemoverSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = AiBackgroundRemoverMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (AiBackgroundRemoverContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = AiBackgroundRemoverMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(AiBackgroundRemoverResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        AiBackgroundRemoverMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (AiBackgroundRemoverContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new AiBackgroundRemoverSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = AiBackgroundRemoverMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new AiBackgroundRemoverResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new AiBackgroundRemoverSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = AiBackgroundRemoverMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = AiBackgroundRemoverMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = AiBackgroundRemoverMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new AiBackgroundRemoverResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, AiBackgroundRemoverDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new AiBackgroundRemoverResult(['ok' => false])]);
        $this->expectException(AiBackgroundRemoverError::class);
        AiBackgroundRemoverDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new AiBackgroundRemoverResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', AiBackgroundRemoverMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        AiBackgroundRemoverMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new AiBackgroundRemoverBaseFeature();
        $b = new AiBackgroundRemoverBaseFeature();
        AiBackgroundRemoverFeatureAdd::call($ctx, $a);
        AiBackgroundRemoverFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): AiBackgroundRemoverBaseFeature
    {
        $f = new AiBackgroundRemoverBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        AiBackgroundRemoverFeatureAdd::call($ctx, self::named_feature('a'));
        AiBackgroundRemoverFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        AiBackgroundRemoverFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        AiBackgroundRemoverFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        AiBackgroundRemoverFeatureAdd::call($ctx, $replace);
        ...
        AiBackgroundRemoverFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): AiBackgroundRemoverContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new AiBackgroundRemoverSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = AiBackgroundRemoverPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        AiBackgroundRemoverPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new AiBackgroundRemoverResponse(['headers' => null]),
            'result' => new AiBackgroundRemoverResult([]),
        ]);
        AiBackgroundRemoverResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        AiBackgroundRemoverResultBody::call($ctx);
